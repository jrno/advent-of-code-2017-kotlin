...
fun main(args: Array<String>) {
    ...
    lines.forEach { line ->
        ...
        val numbers: List<Int> = line.split("\t").map { str -> str.toInt() }

        // part 1
        val low = numbers.min()
        val high = numbers.max()
        high?.let { low?.let { checksum += high-low } } // if not null

        // part 2
        numbers.forEachIndexed({ i, num ->
            numbers.forEachIndexed({ d, divisor ->
                // TODO: could be improved to divide only once
                if (i != d && num % divisor == 0) {
    ...
    println("checksum: $checksum, divisor sum: $divisorsum")
}